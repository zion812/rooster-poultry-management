...
class ProductListingRepositoryImpl @Inject constructor(
    ...
    override fun getProductListings(
        ...
        pageSize: Int, // Used by remoteDataSource
        page: Int // New parameter for Parse pagination
        // lastVisibleTimestamp and lastVisibleDocId are no longer used for Parse
        ...
        return localBackedRemoteResource(
            ...
            remoteCall = {
                // remoteDataSource.getProductListingsStream now returns Flow, but for pagination
                // we want a one-shot fetch per page. We collect the first emission.
                // This still implies that the "Stream" in the DataSource might be better as a suspend function
                // for this specific use case of paginated loading in the repository.
                remoteDataSource.getProductListingsStream(
                    ...
                    pageSize = pageSize,
                    page = page
                ).firstOrNull() ?: Result.Success(emptyList()) // Default if flow is empty
            },
            saveRemoteResult = { listings ->
                val entities = listings.map { mapDomainToEntity(it, needsSync = false) }
                if (page == 1 && !forceRefresh) {
                    // Only clear all if it's the first page of a fresh load (not a refresh of existing data)
                    // More sophisticated cache strategies could be applied (e.g., append if no filters change)
                    // localDataSource.clearAllListings() // Potentially too aggressive
                }
                localDataSource.insertListings(entities) // Upserts
            },
            shouldFetch = { localData ->
                forceRefresh || page > 1 || localData.isNullOrEmpty() // Fetch if refreshing, loading more pages, or cache is empty
            }
