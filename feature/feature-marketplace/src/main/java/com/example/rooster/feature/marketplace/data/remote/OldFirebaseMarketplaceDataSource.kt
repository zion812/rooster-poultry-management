...
class FirebaseMarketplaceDataSource @Inject constructor(
    ...
    override fun getOrdersForUserStream(
        ...
        var query: Query = ordersCollection.whereEqualTo("buyerId", userId)
            .orderBy("orderTimestamp", Query.Direction.DESCENDING)
            .orderBy(FieldPath.documentId(), Query.Direction.DESCENDING) // Secondary sort for stable pagination
            .limit(pageSize.toLong())

        if (lastOrderTimestamp != null && lastOrderId != null) {
            // Fetch the DocumentSnapshot for startAfter
            ordersCollection.document(lastOrderId).get()
                .addOnSuccessListener { lastDocSnapshot ->
                    if (lastDocSnapshot.exists()) {
                        ...
                        // Last visible doc not found, implies reached end or error
                        trySend(Result.Success(emptyList())).isSuccess
                    }
                }
                .addOnFailureListener { e ->
