...
class WorldBankDataFetcher:
    ...
    def fetch_data(self, country_code="all", date_range=None, **kwargs):
        ...
        processed_data = []
        # raw_data is a list: [page_info, list_of_records]
        # We need to iterate over list_of_records, which is raw_data[1]
        actual_records = raw_data[1] if raw_data and len(raw_data) > 1 else []

        for entry in actual_records:
            if entry and entry.get('value') is not None:  # Ensure entry is a dict and has 'value'
