...
# SAMPLE_API_RESPONSE_INFANT_MORTALITY_BRAZIL should be just the records part
SAMPLE_API_RESPONSE_INFANT_MORTALITY_BRAZIL = SAMPLE_API_DATA_RECORDS # Restored

EMPTY_API_RESPONSE_RECORDS = [] # Just the empty list of records
NULL_VALUE_API_RESPONSE_RECORDS = [ # Just the list of records
    {
...
# Removed extraneous ']' from previous line
...
class TestWorldBankDataFetchers(unittest.TestCase):
    ...
    def test_infant_mortality_fetcher(self, MockWorldBankAPI):
        ...
        # Use original complex data for this test
        ...
        self.assertEqual(len(df), 2) # Restored for original data
        self.assertListEqual(list(df.columns), ['Country Name', 'Country Code', 'Year', 'Indicator Name', 'Value'])
        self.assertEqual(df.iloc[0]['Country Name'], "Brazil") # Restored for original data
        self.assertEqual(df.iloc[0]['Country Code'], "BRA") # Restored for original data
        self.assertEqual(df.iloc[0]['Year'], 2019) # Restored for original data (sorted)
        self.assertEqual(df.iloc[0]['Value'], 13.1) # Restored for original data (sorted)
    ...
    def test_fetcher_with_no_api_data(self, MockWorldBankAPI):
        ...
        mock_api_instance = MockWorldBankAPI.return_value
        mock_api_instance.get_data_for_indicator.return_value = EMPTY_API_RESPONSE_RECORDS
    ...
    def test_fetcher_with_null_values_in_data(self, MockWorldBankAPI):
        ...
        mock_api_instance = MockWorldBankAPI.return_value
        mock_api_instance.get_data_for_indicator.return_value = NULL_VALUE_API_RESPONSE_RECORDS
    ...
    def _create_fetcher_test(fetcher_class_to_test):
        ...
        def dynamic_test(self, MockWorldBankAPI):
            ...
            # get_data_for_indicator returns just the list of records
            mock_api_instance.get_data_for_indicator.return_value = sample_records
