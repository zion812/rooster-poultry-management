...
@Singleton
class RealTimeAuctionService @Inject constructor() {
    ...
    private fun handleIncomingMessage(message: String) {
        try {
            ...
            val event = when (eventType) {
                "bid_placed" -> AuctionEvent.BidPlaced(
                ...
                "auction_started" -> AuctionEvent.AuctionStarted(
                    auctionId = data.getString("auctionId"),
                    startTime = data.getLong("startTime"),
                    duration = data.getLong("duration")
                )
                
                "auction_ended" -> AuctionEvent.AuctionEnded(
                    auctionId = data.getString("auctionId"),
                ...
                "participant_joined" -> AuctionEvent.ParticipantJoined(
                    auctionId = data.getString("auctionId"),
                    participantId = data.getString("participantId"),
                    participantName = data.optString("participantName", "Anonymous")
                )
                
                "participant_left" -> AuctionEvent.ParticipantLeft(
                    auctionId = data.getString("auctionId"),
                    participantId = data.getString("participantId")
                )
                
                "time_update" -> AuctionEvent.AuctionTimeUpdate(
                    auctionId = data.getString("auctionId"),
    ...
    fun placeBid(auctionId: String, bidAmount: Double, bidderId: String, bidderName: String) {
        val bidMessage = JSONObject().apply {
