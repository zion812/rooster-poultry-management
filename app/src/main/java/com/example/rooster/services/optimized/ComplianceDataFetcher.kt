...
class ComplianceDataFetcher
    ...
    constructor(
        ...
        fun getLatestRegulations(
            ...
            flow {
                try {
                    ...
                    val regulations =
                        cacheManager.getCachedData<List<Regulation>>(
                            key = cacheKey,
         ...
         */
        fun getCertificationStatus(farmId: String): Flow<List<Certification>> =
            flow {
                try {
                    Log.d(TAG, "Getting certification status for farm: $farmId")

                    val certifications =
                        cacheManager.getCachedData<List<Certification>>(
        ...
        fun getGovernmentSchemes(
            farmType: FarmType,
            location: String,
        ): Flow<List<GovernmentScheme>> =
            flow {
                try {
                    Log.d(TAG, "Getting government schemes for $farmType in $location")

                    val schemes =
                        cacheManager.getCachedData<List<GovernmentScheme>>(
         ...
         */
        fun getComplianceAlerts(farmId: String): Flow<List<ComplianceAlert>> =
            flow {
                try {
        ...
        private fun performComplianceChecks(data: ComplianceData): List<ComplianceCheck> {
            return listOf(
                ...
                ComplianceCheck(
                    ...
                    status =
                        if (data.certifications.any { it.status == CertificationStatus.ACTIVE }) {
