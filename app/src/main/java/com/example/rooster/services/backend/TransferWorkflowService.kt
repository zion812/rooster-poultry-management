...
@Singleton
class TransferWorkflowService @Inject constructor() {
    ...
    suspend fun initializeTransfer(
        ...
        try {
            ...
            val transfer = ParseObject("Transfer").apply {
    ...
    suspend fun submitVerification(
        ...
        try {
            val verification = ParseObject("TransferVerification").apply {
                put("transferId", transferId)
    ...
    private suspend fun saveTransferSteps(transferId: String, steps: List<TransferStep>) {
        steps.forEach { step ->
            val stepObject = ParseObject("TransferStep").apply {
                put("transferId", transferId)
    ...
    private fun isValidStatusTransition(from: TransferStatus, to: TransferStatus): Boolean {
        return when (from) {
            TransferStatus.DRAFT -> to in listOf(TransferStatus.INITIATED, TransferStatus.CANCELLED)
            TransferStatus.INITIATED -> to in listOf(
                TransferStatus.VERIFICATION_PENDING,
                TransferStatus.CANCELLED
            )

            TransferStatus.VERIFICATION_PENDING -> to in listOf(
                TransferStatus.RECEIVER_CONFIRMATION_PENDING,
                TransferStatus.REJECTED
            )

            TransferStatus.RECEIVER_CONFIRMATION_PENDING -> to in listOf(
                TransferStatus.PAYMENT_PENDING,
                TransferStatus.REJECTED
            )

            TransferStatus.PAYMENT_PENDING -> to in listOf(
                TransferStatus.IN_TRANSIT,
                TransferStatus.DISPUTED
            )

            TransferStatus.IN_TRANSIT -> to in listOf(
                TransferStatus.DELIVERED,
                TransferStatus.DISPUTED
            )

            TransferStatus.DELIVERED -> to in listOf(
            ...
            TransferStatus.DISPUTED -> to in listOf(
                TransferStatus.IN_TRANSIT,
                TransferStatus.CANCELLED,
                TransferStatus.COMPLETED
            )
    ...
    private suspend fun updateWorkflowSteps(transferId: String, newStatus: TransferStatus) {
        val stepToActivate = when (newStatus) {
    ...
    private suspend fun sendActionRequiredNotification(transferId: String, status: TransferStatus) {
