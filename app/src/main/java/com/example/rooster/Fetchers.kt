...
fun fetchPersonalMessages(
    ...
    try {
        ...
        val senderQuery = ParseQuery.getQuery<ParseObject>("PersonalMessage")
        senderQuery.whereEqualTo("sender", currentUser)

        val receiverQuery = ParseQuery.getQuery<ParseObject>("PersonalMessage")
        receiverQuery.whereEqualTo("receiver", currentUser)

        val combinedQuery = ParseQuery.or(listOf(senderQuery, receiverQuery))
        combinedQuery.include("sender")
        combinedQuery.include("receiver")
        combinedQuery.orderByDescending("createdAt")
        combinedQuery.limit = 100
        ...
        combinedQuery.findInBackground { objects, e ->
            ...
            if (e != null) {
                ...
                val messages = objects?.mapNotNull { messageObject ->
                    try {
                        val senderUser = messageObject.getParseUser("sender")
                        val receiverUser = messageObject.getParseUser("receiver")

                        PersonalMessage(
                            messageId = messageObject.objectId,
                            senderId = senderUser?.objectId ?: "",
                            senderName = try {
                            ...
                            messageType = MessageType.valueOf(
                                messageObject.getString("messageType") ?: "TEXT"
                            ),
                            isRead = messageObject.getBoolean("isRead"),
                            createdAt = messageObject.createdAt ?: Date(),
                            imageUrl = messageObject.getParseFile("image")?.url,
                            audioUrl = messageObject.getParseFile("audio")?.url
                        )
                    } catch (e: Exception) {
                        null
                    }
...
fun fetchGroupMessages(
    ...
    try {
        ...
        val groupQuery = ParseQuery.getQuery<ParseObject>("GroupChat")
        groupQuery.whereEqualTo("members", ParseUser.getCurrentUser())
        groupQuery.findInBackground { groups, groupError ->
            if (groupError != null) {
                setLoading(false)
                onError(groupError.localizedMessage)
                return@findInBackground
            }

            val groupIds = groups?.map { it.objectId } ?: emptyList()
            if (groupIds.isEmpty()) {
                setLoading(false)
                onResult(emptyList())
                return@findInBackground
            ...
            // Get messages from these groups
            val messageQuery = ParseQuery.getQuery<ParseObject>("GroupMessage")
            messageQuery.whereContainedIn("groupId", groupIds)
                ...
                if (e != null) {
                    ...
                    val messages = objects?.mapNotNull { messageObject ->
                        try {
                            ...
                            GroupMessage(
                                messageId = messageObject.objectId,
                                groupId = messageObject.getString("groupId") ?: "",
                                senderId = senderUser?.objectId ?: "",
                                senderName = try {
                                    senderUser?.username ?: "Unknown"
                                } catch (e: Exception) {
                                ...
                                imageUrl = messageObject.getParseFile("image")?.url,
                                audioUrl = messageObject.getParseFile("audio")?.url,
                                isSystemMessage = messageObject.getBoolean("isSystemMessage")
                            )
                        } catch (e: Exception) {
                            null
                        }
    ...
    try {
        ...
        query.findInBackground { objects, e ->
            ...
            if (e != null) {
                ...
                val messages = objects?.mapNotNull { messageObject ->
                    try {
                        val authorUser = messageObject.getParseUser("author")
                        CommunityMessage(
                            messageId = messageObject.objectId,
                            authorId = authorUser?.objectId ?: "",
                            authorName = try {
                            ...
                            content = messageObject.getString("content") ?: "",
                            category = messageObject.getString("category") ?: "",
                            priority = AnnouncementPriority.valueOf(
                                messageObject.getString("priority") ?: "NORMAL"
                            ),
                            isSticky = messageObject.getBoolean("isSticky"),
                            createdAt = messageObject.createdAt ?: Date(),
                            imageUrl = messageObject.getParseFile("image")?.url,
                            viewCount = messageObject.getInt("viewCount"),
                            likeCount = messageObject.getInt("likeCount")
                        )
                    } catch (e: Exception) {
                        null
                    }
...
// ===============================
// HIGH-LEVEL DASHBOARD FETCHERS
// ===============================

// Fetch high-level dashboard data for admin users
fun fetchHighLevelDashboardData(
    ...
    try {
        ParseCloud.callFunctionInBackground<Map<String, Any>>(
            "getHighLevelDashboard",
            hashMapOf()
        ) { response, e ->
            setLoading(false)
            if (e != null) {
                onError(e.localizedMessage ?: "Failed to fetch dashboard data")
            } else {
                val dashboardData = response?.let { data ->
                    HighLevelDashboardData(
                        totalUsers = (data["totalUsers"] as? Number)?.toInt() ?: 0,
                        activeFarmers = (data["activeFarmers"] as? Number)?.toInt() ?: 0,
                        totalFowl = (data["totalFowl"] as? Number)?.toInt() ?: 0,
                        totalListings = (data["totalListings"] as? Number)?.toInt() ?: 0,
                        dailyActiveUsers = (data["dailyActiveUsers"] as? Number)?.toInt() ?: 0,
                        monthlyRevenue = (data["monthlyRevenue"] as? Number)?.toDouble() ?: 0.0,
                        transfersCount = (data["transfersCount"] as? Number)?.toInt() ?: 0,
                        verificationQueue = (data["verificationQueue"] as? Number)?.toInt() ?: 0,
                        recentActivity = (data["recentActivity"] as? List<Map<String, Any>>)?.map { activityMap ->
                            ActivityItem(
                                userId = activityMap["userId"] as? String ?: "",
                                userName = activityMap["userName"] as? String ?: "",
                                action = activityMap["action"] as? String ?: "",
                                timestamp = parseDate(activityMap["timestamp"] as? String)
                                    ?: Date(),
                                details = activityMap["details"] as? String ?: ""
                            )
                        } ?: emptyList(),
                        topFarmers = (data["topFarmers"] as? List<Map<String, Any>>)?.map { farmerMap ->
                            TopFarmer(
                                farmerId = farmerMap["farmerId"] as? String ?: "",
                                farmerName = farmerMap["farmerName"] as? String ?: "",
                                fowlCount = (farmerMap["fowlCount"] as? Number)?.toInt() ?: 0,
                                totalSales = (farmerMap["totalSales"] as? Number)?.toInt() ?: 0,
                                revenue = (farmerMap["revenue"] as? Number)?.toDouble() ?: 0.0,
                                region = farmerMap["region"] as? String ?: "",
                                rank = (farmerMap["rank"] as? Number)?.toInt() ?: 0
                            )
                        } ?: emptyList(),
                        systemHealth = SystemHealthStatus(
                            serverStatus = data["serverStatus"] as? String ?: "UNKNOWN",
                            databaseHealth = (data["databaseHealth"] as? Number)?.toDouble() ?: 0.0,
                            apiResponseTime = (data["apiResponseTime"] as? Number)?.toDouble()
                                ?: 0.0,
                            errorRate = (data["errorRate"] as? Number)?.toDouble() ?: 0.0
                        ),
                        lastUpdated = Date()
                    )
                } ?: HighLevelDashboardData()
...
// ===============================
// SEARCH & DISCOVERY FETCHERS
// ===============================

// Fetch search results across multiple categories
    ...
    try {
        ...
        ParseCloud.callFunctionInBackground<List<Map<String, Any>>>(
            ...
            if (e != null) {
                ...
                val searchResults = response?.map { resultMap ->
                    SearchResult(
                        id = resultMap["id"] as? String ?: "",
                        title = resultMap["title"] as? String ?: "",
                        description = resultMap["description"] as? String ?: "",
                        category = SearchCategory.valueOf(
                            resultMap["category"] as? String ?: "FOWL"
                        ),
                        imageUrl = resultMap["imageUrl"] as? String,
                        price = (resultMap["price"] as? Number)?.toDouble(),
                        location = resultMap["location"] as? String,
                        relevanceScore = (resultMap["relevanceScore"] as? Number)?.toDouble()
                            ?: 0.0,
                        isVerified = resultMap["isVerified"] as? Boolean ?: false,
                        sellerId = resultMap["sellerId"] as? String,
                        sellerName = resultMap["sellerName"] as? String
                    )
                } ?: emptyList()
...
// ===============================
// EVENTS & COMPETITIONS FETCHERS
// ===============================

// Fetch upcoming events and competitions
fun fetchUpcomingEvents(
    ...
    try {
        ...
        query.whereGreaterThan("eventDate", Date()) // Future events only
        region?.let { query.whereEqualTo("region", it) }
        ...
        query.findInBackground { objects, e ->
            ...
            if (e != null) {
                ...
                val events = objects?.mapNotNull { eventObject ->
                    try {
                        val organizerUser = eventObject.getParseUser("organizer")
                        ParseEvent(
                            ...
                            eventType = EventType.valueOf(
                                eventObject.getString("eventType") ?: "COMPETITION"
                            ),
                            eventDate = eventObject.getDate("eventDate") ?: Date(),
                            location = eventObject.getString("location") ?: "",
                            region = eventObject.getString("region") ?: "",
                            organizerId = organizerUser?.objectId ?: "",
                            organizerName = try {
                                organizerUser?.username ?: "Unknown"
                            } catch (e: Exception) {
                                "Unknown"
                            },
                            maxParticipants = eventObject.getInt("maxParticipants"),
                            currentParticipants = eventObject.getInt("currentParticipants"),
                            entryFee = eventObject.getDouble("entryFee"),
                            prizeMoney = eventObject.getDouble("prizeMoney"),
                            registrationDeadline = eventObject.getDate("registrationDeadline"),
                            isRegistrationOpen = eventObject.getBoolean("isRegistrationOpen"),
                            category = eventObject.getString("category") ?: "",
                            imageUrl = eventObject.getParseFile("image")?.url,
                            createdAt = eventObject.createdAt ?: Date()
                        )
                    } catch (e: Exception) {
                        null
                    }
...
// TODO: Commented out advanced fetchers to restore build. Re-enable as needed.
*/
